...
\vspace{-3pt}
\resumeItemListStart
\resumeItem{Implemented and benchmarked statistical models (Seasonal Autoregression, Vector Autoregression, Facebook Prophet) and deep learning based models (Long-Short Term Memory neural networks, Temporal convolutional networks) for forecasting demand data across different nodes in pharmaceutical supply chains, achieving a \textbf{90\% reduction in Mean Absolute Percentage Error}.}
% \vspace{-3pt}
    \resumeItem{Conducted time-series diagnostics \textbf{Autocorrelation \& Partial autocorrelation plots \& extended autocorrelation matrices, mean and variance stationarity tests}for detemining optimal ARIMA order and incorporated \textbf{exogenous variables} like mortality rates, veteran population statistics, inventory, backorders into forecasting models to improve forecast accuracy.}
% \vspace{-3pt}
    \resumeItem{Built an \textbf{end-to-end forecasting pipeline} including data generation, pre-processing, model selection with custom model fit metrics, and docker-based containerization for reproducible deployment.}
    % \vspace{-3pt}
